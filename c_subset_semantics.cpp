...
struct Ty { struct Int{}; struct Char{}; struct Enum{std::string name;}; struct Void{}; struct Ptr{T to;}; struct Struct{std::string name;}; struct Fn{std::vector<T> args; T ret;}; using N=std::variant<Int,Char,Enum,Void,Ptr,Struct,Fn>; N n; template<class X> Ty(X x):n(std::move(x)){} };
T integer(){return std::make_shared<Ty>(Ty::Int{});} T character(){return std::make_shared<Ty>(Ty::Char{});} T enum_type(std::string n){return std::make_shared<Ty>(Ty::Enum{std::move(n)});} T unit(){return std::make_shared<Ty>(Ty::Void{});}
...
bool integerish(T const&t){return std::holds_alternative<Ty::Int>(t->n)||std::holds_alternative<Ty::Enum>(t->n);}
...
enum class BinOp { Add, Equal, NotEqual, Subtract, Multiply, Less, Greater, LessEqual, GreaterEqual, Modulo, Divide, LogicalAnd, LogicalOr, BitAnd, BitOr, BitXor, ShiftLeft, ShiftRight };
...
bool lvalue(Expr const&x){return std::holds_alternative<Var>(x->n)||std::holds_alternative<Deref>(x->n)||std::holds_alternative<Member>(x->n)||std::holds_alternative<Arrow>(x->n)||std::holds_alternative<FunctionRef>(x->n);}
...
void check_switch(Expr const&selector,std::vector<SwitchCase> const&cases,std::vector<Stmt> const&default_body,T const&ret,Env const&env,Functions const&fs,StructFields const&ss,int loop_depth=0){if(!integerish(infer(selector,env,fs,ss)))throw std::runtime_error("switch selector must be integer-like");std::vector<int> labels;for(auto const&c:cases){if(!same(infer(c.label,env,fs,ss),integer()))throw std::runtime_error("case label must be integer");int label=constant_label(c.label);if(std::find(labels.begin(),labels.end(),label)!=labels.end())throw std::runtime_error("duplicate case label");labels.push_back(label);check_statements(c.body,ret,env,fs,ss,loop_depth);}check_statements(default_body,ret,env,fs,ss,loop_depth);}
}
...
int main(){using namespace csem;try{
  ...
  auto char_callback=function({Char},Char); Env char_callback_env{{"cb",pointer(char_callback)},{"c",Char}};
  if(!same(infer(assign(variable("cb"),address(function_ref(char_identity.name))),char_callback_env,char_functions,{}),pointer(char_callback))||!same(infer(indirect_call(dereference(variable("cb")),{variable("c")}),char_callback_env,char_functions,{}),Char))throw std::runtime_error("char callback signature failed");
  ...
  if(!same(infer(binary(BinOp::Multiply,literal(3),literal(4)),{}, {}, structs),integer()))throw std::runtime_error("multiplication operator type failed");
  ...
  if(!same(infer(indirect_call(arrow(arrow(variable("thunk"),"next"),"run"),{literal(9)}),{{"thunk",pointer(Thunk)}},callback_fs,callback_structs),integer()))throw std::runtime_error("recursive callback chain call failed");
  ...
  Function sink{"sink",{{"x",integer()}},unit(),{literal(0)}};
  Functions void_callback_fs{{"sink",&sink}};
  auto void_callback_type=function({integer()},unit());
  Function consume_callback{"consume_callback",{{"cb",void_callback_type},{"x",integer()}},unit(),{indirect_call(variable("cb"),{variable("x")})}};
  check(consume_callback,void_callback_fs,structs);
  if(!same(infer(indirect_call(variable("cb"),{literal(3)}),{{"cb",void_callback_type}},void_callback_fs,structs),unit()))throw std::runtime_error("void callback call failed");
