...
void check_program(std::vector<Function> const&program,StructFields const&ss){validate_structs(ss);Functions fs;for(auto const&f:program)if(!fs.emplace(f.name,&f).second)throw std::runtime_error("duplicate function "+f.name);for(auto const&f:program)check(f,fs,ss);}
