...
using P=std::variant<True,Name,Path,Kind,Size,Empty,std::shared_ptr<And>,std::shared_ptr<Or>,std::shared_ptr<Not>>;
...
void walk(const fs::path&root,const P&q,int depth,int maxdepth,int mindepth,bool print0){std::error_code ec;fs::directory_entry e(root,ec);if(ec)throw std::runtime_error("cannot access "+root.string());if(depth>=mindepth&&eval(q,e))std::cout<<root.string()<<(print0?'\0':'\n');if(!e.is_directory(ec)||(maxdepth>=0&&depth>=maxdepth))return;for(auto const&x:fs::directory_iterator(root,ec))if(!ec)walk(x.path(),q,depth+1,maxdepth,mindepth,print0);}
}
