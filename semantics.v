...
Lemma typed_if_nonzero_semantic_agreement : forall M Γ σ e st sf n σ',
  cmstmt_typed Γ (CMIf e st sf) CVoid ->
  cexpr_big M σ e (CVInt (S n)) ->
  cmstmt_big M σ st None σ' ->
  cmconfig_typed Γ (st, σ') ->
  cmstmt_big M σ (CMIf e st sf) None σ' /\
  cmstmt_step_star M (CMIf e st sf, σ) (st, σ) /\
  cmconfig_typed Γ (st, σ').
Proof.
...
Lemma typed_while_zero_unfold : forall M Γ σ e st,
...
Lemma typed_while_nonzero_semantic_agreement : forall M Γ σ e st n σ1 σ2,
  cmstmt_typed Γ (CMWhile e st) CVoid ->
  cexpr_big M σ e (CVInt (S n)) ->
  cmstmt_big M σ st None σ1 ->
  cmstmt_big M σ1 (CMWhile e st) None σ2 ->
  cstore_typed Γ σ ->
  cmstmt_big M σ (CMWhile e st) None σ2 /\
  cmstmt_step_star M (CMWhile e st, σ)
    (CMIf e (CMSeq st (CMWhile e st)) CMSkip, σ) /\
  cmconfig_typed Γ
    (CMIf e (CMSeq st (CMWhile e st)) CMSkip, σ).
Proof.
  intros M Γ σ e st n σ1 σ2 Hwhile He Hbody Hrest Hσ.
  pose proof (typed_while_nonzero_unfold M Γ σ e st n Hwhile He Hσ)
    as [Hunfold Htyped].
  split.
  - eapply CMBWhileStep; eauto.
  - split.
    + now apply cmstmt_step_to_star.
    + exact Htyped.
Qed.
