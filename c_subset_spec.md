...
executes the mandatory first iteration exactly once.
...
Four independently checked writes in an inclusive post-test body are covered
by `do_write_inclusive_four_body_writes.c`.
Five independently checked writes in an inclusive post-test body are covered
...
`do_write_five.c`.
Post-test bodies may contain two independently checked writes, covered by
...
Two adjacent fragments in an inclusive while payload are covered by
`while_write_inclusive_adjacent_binary.c`.
The same payload is accepted with an unbraced explicit increment in
...
Three adjacent fragments in a strict braced body are covered by
`loop_write_braced_three_adjacent_binary.c`.
Four adjacent fragments are covered by
...
Four adjacent fragments with inclusive bounds are covered by
`loop_write_inclusive_braced_four_adjacent_binary.c`.
Five adjacent fragments complete the inclusive braced ladder in
`loop_write_inclusive_braced_five_adjacent_binary.c`.
